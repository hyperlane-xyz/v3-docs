...
3. Tokens
 1. There are no native tokens, this has implications to the IGP where a `denom` for gas payments has to be specified. Similarly, there there is no `NativeCollateralToken`.

### Tool integration

Although it’s in development, Hyperlane agents and CLI do not yet support the native Cosmos SDK module. Deploying a Warp Route requires manually setting up all necessary components (Mailbox, ISM, IGP, Token, etc.) with the correct transactions.
