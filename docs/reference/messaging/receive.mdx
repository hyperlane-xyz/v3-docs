...
{/* hint style="danger" */}
To ensure only valid interchain messages are accepted, it is important to require that `msg.sender` is a known Hyperlane [Mailbox](./messaging-interface.mdx).
{/* endhint */}

Developers must implement access control on `handle()` in order to ensure the security of interchain messages. Only a Hyperlane [Mailbox](./messaging-interface.mdx) contract should have permission to call this function. An example of this access control is shown below.
