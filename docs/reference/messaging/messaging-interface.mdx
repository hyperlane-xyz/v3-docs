...
Hyperlane provides an onchain API for sending and receiving interchain messages.

## Interface

To [**send**](./send.mdx) interchain messages, reference the `Mailbox.dispatch()` interface.

To [**receive**](./receive.mdx) interchain messages, implement the `IMessageRecipient` interface.
...
Learn more about [paying for interchain gas](../hooks/interchain-gas.mdx).