...
:::warning
Hyperlane can only deliver messages to smart contracts that implement the `IMessageRecipient` interface. See the [Handle](./receive.mdx) documentation for more information.
:::
...
There are two Mailbox hooks that are configured by the `owner`: required and default. The required hook will be invoked for all `dispatch` calls and the default hook will be invoked unless overriden. See the [Hooks](../hooks/overview.mdx) documentation for more information.
