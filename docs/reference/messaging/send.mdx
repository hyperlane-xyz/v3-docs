...
根据 [post-`dispatch` hook configuration](##post-dispatch-hook-config)，可能需要一些付款。更多信息请参阅 [`quoteDispatch`](#Quote-Dispatch)。
...
:::tip
对`dispatch` 少付的金额将回滚，但多付的金额将退还给消息发送者。 这样，开发人员就可以跳过显式的`quoteDispatch`调用，转而使用具有超额值的`quoteDispatch`调用。
:::
