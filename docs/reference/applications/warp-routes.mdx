...
Unlike other token wrapping protocols, Warp Routes are secured by Hyperlane's modular [security](../protocol/sovereign-consensus/ "mention") protocol, allowing developers to specify the security model that governs the minting, burning, and unwrapping of their interchain token.
...
Each Warp Route consists of one contract deployed on every chain that the token can move between. These contracts use the [Mailbox](../apis/messaging-api/ "mention") to send interchain messages to one another.&#x20;
