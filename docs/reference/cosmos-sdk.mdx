...
:::warning
**Current Status:** The Hyperlane Cosmos SDK Module is currently under development and will be **released soon**!
:::

## Overview

The Hyperlane Cosmos SDK Module allows Cosmos SDK-based chains to integrate Hyperlane for cross-chain messaging. It provides a permissionless way for Cosmos chains to communicate with external ecosystems, including EVM and other ecosystems where [Hyperlane is deployed on](https://docs.hyperlane.xyz/docs/reference/addresses/mailbox-addresses).
...
- **Built by KYVE**: Developed by the [KYVE](https://www.kyve.network/) team to extend Cosmos interoperability beyond IBC, starting with bridging the $KYVE token to EVM.
