...
Follow the instructions in [Agent Keys](../set-up-agent-keys.mdx) to generate an AWS IAM user and either a KMS or a hexadecimal key. You will use this key's credentials in the following steps. Note that the IAM user is required for setting up the S3 bucket, so is required regardless of the type of key used.
