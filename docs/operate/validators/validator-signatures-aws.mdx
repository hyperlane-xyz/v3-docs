...
如果您只打算为测试或开发目的运行验证器，请考虑按照[local setup guide](../../guides/deploy-hyperlane-local-agents.mdx)进行操作。
...
高级用户可以考虑使用[S3 terraform module](../deploy-with-terraform.mdx#iam--kms)来创建具有正确权限的S3存储。
