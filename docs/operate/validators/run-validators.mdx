...
- Publicly readable storage
  - Validators write their signatures off-chain to publicly accessible, highly available, storage, so that they can be aggregated by the [relayer](../../protocol/agents/relayers.mdx).
...
This is intended to show how you can run a validator on your _local machine_, which is only for testing and development purposes. This also means that signatures from local validators are only able to be accessed by a locally ran relayer.
