...
3. The same checkpoint syncer S3 bucket can by used by multiple validators, however they **must** use different folders, e.g. using the `--checkpointSyncer.folder` flag.
