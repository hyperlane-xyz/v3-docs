...
This signature is stored and made publicly available (e.g. in a S3 bucket), which is then used by the offchain Relayer and Interchain Security Modules onchain. Validators are _not_ networked together and do not need to reach consensus; they also do not regularly submit transactions.
...
- The design is [open source](https://github.com/hyperlane-xyz/hyperlane-monorepo/tree/56be527d691a11a1ff2de4a390fd0dae8bb77347/typescript/infra/src/agents) and generalizable to other storage and key solutions. There is a community-submitted [PR for GCS](https://github.com/hyperlane-xyz/hyperlane-monorepo/pull/3156).
...
:::note[Note for Chain Deployers - not related to Validator Operators]
...
Experienced operators may prefer to [deploy agents with terraform](../operate/run-docker-compose.mdx) instead. This process will automatically create agent keys, Validator buckets, permissions and any other auxiliary setup required to run a Validator cluster on AWS.
...
<TabItem value="production-setup-cosmos" label="Production Setup (Cosmos)">
