...
If you're an deploying Hyperlane to a new chain, follow the [local agents guide](../../guides/deploy-hyperlane-local-agents.mdx) to understand how to configure and run a Validator locally, which is **only for testing and development purposes**. Signatures from local Validators are only able to be accessed by a locally run Relayer. For production, continue with this guide below using publicly-available storage, i.e. the S3 Bucket.
...
Experienced operators may prefer to [deploy agents with terraform](.././run-docker-compose.mdx) instead. This process will automatically create agent keys, Validator buckets, permissions and any other auxiliary setup required to run a Validator cluster on AWS.
