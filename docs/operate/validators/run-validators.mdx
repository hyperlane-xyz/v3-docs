...
- For an introduction on Validators in Hyperlane, you can check out the [Overview section](/docs/operate/overview-agents.mdx).
  :::
...
- **Secure Signing Key**
  ...
  - The Hyperlane Validator agent currently supports signing with AWS KMS keys that are accessed via API keys/secrets as well as hexadecimal plaintext keys. See more under [agent keys](../set-up-agent-keys.mdx).
...
- **RPC Node**
  - Validators make simple view calls to read merkle roots from the [Mailbox](../../protocol/mailbox.mdx) contract on the chain they are validating for. You must use your own RPC urls not any public ones.
...
Experienced operators may prefer to [deploy agents with Terraform](../deploy-with-terraform.mdx) or a community-submitted [Ansible playbook](https://github.com/polkachu/hyperlane) instead. This process will automatically create agent keys, Validator buckets, permissions and any other auxiliary setup required to run a Validator cluster on AWS.
...
- **Configure multiple RPCs:** Use [`customRpcUrls`](../config-reference.mdx#chainschain_namecustomrpcurls).
- **RPC Selection Mode ([`rpcConsensusType`](../config-reference.mdx#chainschain_namerpcconsensustype))**:
...
- **Configure multiple RPCs:** Use [`customRpcUrls`](../config-reference.mdx#chainschain_namecustomrpcurls).
- **Configure gRPC endpoints:** Use [`customGrpcUrls`](../config-reference.mdx#chainschain_namecustomgrpcurls).
...
- **Configure RPCs:** Use [`customRpcUrls`](../config-reference.mdx#chainschain_namecustomrpcurls).

:::caution

For **SVM chains**, only a **single RPC URL** is used. here is no fallback or quorum-based selection mechanism.
...
We encourage folks to validate on as many chains as they are interested in supporting. This section provides a detailed explanation of the benefits, requirements, and instructions to set up multiple validators.

**Benefits of Running Multiple Validators**

- **Enhanced Security**: Deploying multiple validators across different chains or regions reduces the risk of a single point of failure.
- **Increased Redundancy**: Multiple validators ensure that if one validator encounters issues or downtime, others can continue to operate, maintaining the integrity and availability of the validation process.

### Running Mulitple Validators: Key Considerations 
...
#### 1. Configure Each Validator

- Generate an agent configuration file for each validator, specifying the chains they will validate and the corresponding RPC URLs.
