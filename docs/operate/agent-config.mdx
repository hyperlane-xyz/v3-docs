...
* `{ "chains": { "avalanche": { "rpcUrls": [ { "http": "https://some-url.com" } ] } } }` (abbreviated as `chains.<chain_name>.rpcurls.0.http`) can be set with `--chains.avalanche.rpcurls.0.http "https://some-url.com"`
...
* `{ "chains": { "avalanche": { "rpcUrls": [{ "http": "https://some-url.com" }] } } }` (abbreviated as `chains.avalanche.rpcurls.0.http` or `chains.<chain_name>.rpcurls.0.http`) can be set with `HYP_CHAINS_AVALANCHE_RPCURLS_0_HTTP="https://some-url.com"` or `HYP_VALIDATOR_AVALANCHE_RPCURLS_0_HTTP="https://some-url.com"` and so on...
