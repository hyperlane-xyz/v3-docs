...
METRICS_PORT=9090 # external port, default is 9090 and should be different for each validator ie 9091, 9092, ...
...
```bash
docker compose --env-file .env.ethereum up validator1 -d
docker compose --env-file .env.optimism up validator2 -d
docker compose --env-file .env.base up validator3 -d
```
