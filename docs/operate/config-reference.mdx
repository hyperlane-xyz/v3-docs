...
Connections to other VMs only support a single RPC connection. If more than one RPC url is specified in [rpcUrls](#chainschain_namerpcurls), only the first one will be used.
...
Connections to VMs other than EVM only support a single RPC connection. If more than one RPC url is specified, only the first one will be used.

:::

**Description:** Urls to rotate between in case of RPC failure. 
