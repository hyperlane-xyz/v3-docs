...
**The `quorum` consensus type currently only applies to EVM chains.**

`single` and `fallback` mode is supported on EVM, CosmWasm and Sealevel providers. 
Connections to other Alt-VM chains only support a single RPC url. If more than one RPC url is specified in [rpcUrls](#chainschain_namerpcurls), only the first one will be used.
...
**Optional:** Yes (Will use `defaultRpcConsensusType` if not specified)
...
**`defaultRpcConsensusType` is only used for EVM providers.**

The `quorum` consensus type currently only applies to EVM chains. `single` and `fallback` mode is supported on EVM, CosmWasm and Sealevel providers. 
Connections to other Alt-VM chains only support a single RPC url. If more than one RPC url is specified in [rpcUrls](#chainschain_namerpcurls), only the first one will be used.
...
<TabItem value="arg" label="As Arg" default>
```bash
--defaultRpcConsensusType single
```
</TabItem>
