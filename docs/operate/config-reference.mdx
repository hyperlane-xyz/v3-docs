...
这些文件必须在代理程序可以访问的文件系统中可访问。如果您正在运行 Docker，请参阅[代理配置指南](./agent-config.mdx#config-files-with-docker)中有关将配置文件挂载到 Docker 容器中的提示。
...
**说明：** 用于唯一标识域的超平面域 ID。另请参阅：[域标识符](../reference/domains.mdx)。
