...
在您的工作目录中，可能会发现一个包含 `chains.yaml` 文件的 `config/` 文件夹。通过该 `chains.yaml` 配置文件，您可以描述链元数据，以便在 Hyperlane 部署或应用程序中使用。
...
您可以通过扩展核心链配置来覆盖 RPC 网址。

在下面的示例中，你可以看到如何定义 RPC 数组，以及如何调整其中任何一个 RPC 的重试参数。
...
在下面的示例中，我们使用的燃料价格为7gwei，对 nonce 进行了硬编码，并为基本费和优先费设置了最大值。
