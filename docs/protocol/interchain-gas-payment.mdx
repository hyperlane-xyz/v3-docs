...
These contracts expose the [InterchainGasPaymaster interface](../reference/hooks/interchain-gas.mdx), which allow users to pay the [relayer](./agents/relayer.mdx) with native tokens on the origin chain to cover the costs of delivering a message on the destination chain.
...
The gas limit is the cost of calling `handle` on the destination chain for a given message. This can vary depending on message content and the logic of the handler.

### Gas overhead

The gas overhead is set as part of the destination gas configuration, along with the `gasOracle`:
...
This corresponds to the operation cost of processing a message on the destination, irrespective of the message itself.
