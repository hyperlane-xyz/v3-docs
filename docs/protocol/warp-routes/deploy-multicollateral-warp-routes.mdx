...
:::note
When sending between two `collateral` chains (e.g., Base → Arbitrum), make sure there’s sufficient collateral available on the destination chain to complete the transfer.

Synthetic tokens are minted on the destination. In collateral routes, tokens must already exist on the destination chain in order to be released.
:::
