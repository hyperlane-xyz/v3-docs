...
Each Warp Route requires a contract deployed on every chain its on that acts as an entry/exists point. These contracts use the [Mailbox](../mailbox.mdx) to send interchain messages to one another.
