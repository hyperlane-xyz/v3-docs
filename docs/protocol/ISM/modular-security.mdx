...
The first, `_metadata`, consists of arbitrary bytes provided by [Relayer](../agents/relayers.mdx "mention"). Typically, these bytes are specific to the ISM. For example, for a [MultisigISM](multisig-ISM.mdx "mention"), `_metadata` must include validator signatures.
...
The secondary function that ISMs must implement is `moduleType()`. This is used to signal to [Relayer](../agents/relayers.mdx "mention") what to include in `_metadata`. ISMs **must** return one of the supported module types.&#x20;
