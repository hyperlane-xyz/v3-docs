...
`AggregationISMs` can aggregate the security of any ISMs. For example, users can deploy a [MultisigISM](multisig-ISM.mdx) with their own validator set, and deploy an `AggregationISM` that aggregates that ISM with the Hyperlane default ISM.
...
For example, a custom implementation could require that low value messages be verified by a [MultisigISM](multisig-ISM.mdx), and require that high value messages _also_ be verified by a [WormholeISM](./wormhole-ISM.mdx).
