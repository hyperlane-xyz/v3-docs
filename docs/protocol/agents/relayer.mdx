...
:::note
Running your own relayer involves deploying an [IGP contract](../interchain-gas-payment.mdx) and maintaining a gas price inside of it. Therfore, we only recommend doing this while setting up a permissionlessly deployed bridge, rather than reusing an existing bridge.
:::
...
The relayer will periodically retry messages when processing fails. Messages that fail to process on the first attempt will cause the relayer to retry with exponential backoff. After a maximum amount of retries, the relayer will no longer attempt to process the message.
