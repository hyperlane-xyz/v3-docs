...
The relayer's job is to deliver messages from origin to destination chains. Depending on the [Interchain Security Module (ISM)](../../reference/ISM/specify-your-ISM) specified by the recipient, metadata may be required to prove the security of the message, such as Validator signatures for the [Multisig ISM](./ISM/multisig-ISM.mdx), merkle proofs, zero-knowledge proofs, and more. So before attempting to deliver, the relayer gathers any required metadata.

Operating a relayer involves making sure its balance is high enough on all destination chains, to enable delivery transactions to be sent. Message senders are charged for delivery on the origin chain, so it is the relayer operator's responsibility to rebalance revenue to destination chain accounts.

:::tip
The relayer will periodically retry metadata gathering and message submission. This is important to improve resilience against disruptins, such as: no gas payment made to pay for delivery, insufficient relayer balance, temporary spikes in transaction fees on the destination, validator downtime.
:::
...
While permissionless to run, it's up to the message sender to pick which relayer to pay for delivery. [Mailbox](./mailbox.mdx) integrations usually default to always paying the same relayer, so if driven by economic incentives, it's only recommended to run a relayer for a self-deployed bridge.

To read chain state and send transactions, chain connections are established via RPC.
...
// TODO: insert table about data being indexed, chains where this happens, and guarantees
