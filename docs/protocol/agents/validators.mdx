...
Validators are responsible for observing the [Mailbox](../mailbox.mdx) contract and, if required, signing its merkle root to facilitating message transmission to remote chains.

Unlike many other protocols, Hyperlane does **not** have an enshrined validator set. Anyone is free to run their own validator, as long as the receiver contract specifies a [Multisig ISM](../ISM/multisig-ISM.mdx) includes their validator.
