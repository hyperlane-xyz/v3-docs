...
Validators are responsible for observing the [Mailbox](/docs/protocol/mailbox) contract and, if required, signing its merkle root to facilitating message transmission to remote chains.
...
:::info
Want to run a validator? Follow the [validators guide](/docs/operate/validators/run-validators).
:::

For messages using a [Multisig ISM](/docs/reference/ISM/multisig-ISM-interface), validators read the current merkle root by calling `MerkleTreeHook.latestCheckpoint()`.  Once a root has achieved sufficient [finality](https://medium.com/mechanism-labs/finality-in-blockchain-consensus-d1f83c120a9a), validators are expected to sign it and post their signature to highly available storage so that it can be aggregated by [relayers](/docs/protocol/agents/relayers).
