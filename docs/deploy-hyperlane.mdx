...
To go to production, ensure you’ve successfully pushed a registry PR (the last step) and reach back over telegram and we can add your custom chain to the core chains we have in the CLI and relay for.
...
## 2) Registry: add your custom chain
...
<details>
  <summary>
    After successfully creating a new core deployment config, you can find it in a file named `core-config.yaml` within  `./configs`
  </summary>
...
Currently core self-relay only works from **an existing origin chain** (holesky is recommended) **to your custom chain**. When dispatch is successful, you should see that the message was self-relayed!

:::note
**🎉 Congrats! You have successfully sent a message to your custom chain**
:::

## 4) Warp Route: configure & deploy your custom chain’s warp route contracts
...
2. Select the chains you want to set up a warp route between, e.g.  `yourchain, holesky`
...
<details> 
  ...
  Under `$HOME/.hyperlane/deployments/warp_routes/` you will find a new folder named with your custom or other chain’s native token name, and two files named `otherchain-yourchain-addresses.yaml` & `otherchain-yourchain-config.yaml` within that folder
    
    1. Example contents of this config at `$HOME/.hyperlane/deployments/warp_routes/AVAX/holesky-yourchain-addresses.yaml` based on the above examples:
    ...
    2. Example contents of this config at `$HOME/.hyperlane/deployments/warp_routes/AVAX/holesky-yourchain-config.yaml` based on the above examples:
...
:::note
**💸 Well done, a Hyperlane warp route is now deployed to your custom chain.**
:::
