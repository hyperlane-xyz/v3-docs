...
Use the Hyperlane CLI again to send the message. The CLI will ask for a set of core deployment artifacts to use. Select the `core-deployments` JSON file from Step 2.
...
The Warp UI is a DApp template for interacting with Warp Routes. You can clone the repo at [https://github.com/hyperlane-xyz/hyperlane-warp-ui-template](https://github.com/hyperlane-xyz/hyperlane-warp-ui-template) and follow the instructions in CUSTOMIZE.md to configure and a new instance. In short, use the `chains.yaml` and `warp-ui-token-config.json` files from the previous steps to provide the UI with the information it needs.
