...
3. [<b>Run validators and relayers </b>](#3-run-validators-relayers) using [Kurtosis](https://cloud.kurtosis.com). Validators to provide the signatures needed for the interchain Security Modules you deployed in step 2 and relayers to send and receive messages between chains you deployed contracts to in step 2.
4. [<b>Send a test message</b>](#5-send-test-messages) to confirm that your relayer is able to deliver messages from and to each pair of chains
...
We've partnered with [Kurtosis](https://www.kurtosis.com/) to provide a smooth validator and relayer deployment experience using [Kurtosis Cloud](https://cloud.kurtosis.com/).

Visit the [Kurtosis Cloud website](https://cloud.kurtosis.com/) (direct link: https://cloud.kurtosis.com/) and sign in with Google. 

If this is your first time using Kurtosis Cloud, Kurtosis will provision a remote cloud instance for your validator and relayer. Provisioning only take a few minutes.
...
A configuration screen will pop up where you'll enter your relayer and validator information created in Steps 1 and 2. Fields with a red asterik are required such as the `Origin Chain Name`, `Validator Key`, `Agent Config Json` and `Relay Chains`. The screenshot below shows an example configuration. Click `Run` to deploy your relayer and validator!

:::warn

Kurtosis Cloud is currently in beta and does not yet support secrets management! Thus, only use Kurtosis Cloud for testing purposes and do not enter any production secrets!
...
You can now see information about your relayer and validator deployment by navigating to `Overview`. You can also view the relayer and validators logs along with other information about the services (see screenshots below).

![Enclave Overview](../static/img/kurtosis-enclave-overview.png)

![Validator Logs](../static/img/kurtosis-validator-logs.png)

For feedback or questions about using Kurtosis in this section, please file a [Github Issue](https://github.com/kurtosis-tech/kurtosis/issues/new/choose) or start a [Github Discussion](https://github.com/kurtosis-tech/kurtosis/discussions/new?category=q-a). Additionally, the Kurtosis team is available for technical support on their [Discord server](https://discord.com/invite/jJFG7XBqcY).

:::info

Kurtosis Cloud is available for a one month free trial. To continue using Kurtosis for deploying your setup beyond the trial, you can provide your payment information [https://cloud.kurtosis.com/payment].
