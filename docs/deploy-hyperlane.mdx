...
To run the Hyperlane agents yourself:
...
- Follow the [Relayer](./operate/relayer/run-relayer.mdx) guide to run a relayer that will deliver interchain messages sent between the local and remote chains. Remember to set `--relayChains` or `HYP_RELAYCHAINS` appropriately.

Don't forget to include the agent config from the deploy contracts step in `CONFIG_FILES`.
If you are using Docker, you will need to mount the file into the container.
You should have already set up your validator and relayer keys, so you can skip that part of the guide.

Once the Validators and Relayer are running, you can skip to Step 4 and start sending test messages!

:::warning

Make sure these validators match the addresses you provided when creating your ISM config. Otherwise, the ISM you deployed in the previous step will not be able to verify messages sent from your chain.
