...
Relayers can be defined with multiple origin chains, therefore only one relayer is required to operate across a set of chains.

If you are deploying more than one Kurtosis enclave, ensure that the set of relay chains for each enclave does not overlap.
