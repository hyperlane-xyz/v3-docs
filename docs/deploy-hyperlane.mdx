...
Follow the [Validators](./operate/run-validators.mdx) guide to run validators for the Mailbox on your local chain. These validators will provide the security for messages sent _from_ your chain to remote chains.
...
You should have already set up your validator keys in [step 1](#1-set-up-keys), so you can skip that part of the [Validators](./operate/run-validators.mdx) guide.
...
Follow the [Relayer](./operate/run-relayer.mdx) guide to run a relayer that will deliver interchain messages sent between the local and remote chains.

Remember to set `--relayChains` or `HYP_BASE_RELAYCHAINS` appropriately.


Include the agent config from the [deploy contracts](#2-deploy-contracts) step in `CONFIG_FILES`. If you are using Docker, you will need to mount the file into the container.
You should have already set up your relayer keys in [step 1](#1-set-up-keys), so you can skip that part of the [Relayer](./operate/run-relayer.mdx) guide.
