...
The owner and controller should typically be set to a Safe multisig for enhanced security, the minter is the Warp Route contract address on the local chain.

Mint limits for `FiatToken` are absolute, meaning they do not reset automatically and must be updated if required.
