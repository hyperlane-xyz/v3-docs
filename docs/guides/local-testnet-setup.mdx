...
On a new termial, use the Hyperlane CLI to create a custom chain config:

```bash
hyperlane registry init
```

Follow the prompts to set up `anvilchain1`. The CLI will ask you for the details of your chains including chainId and RPC URLs. Repeat the process for `anvilchain2`.

This process will create `metadata.yaml` files under `$HOME/.hyperlane/chains/anvilchain1` and `$HOME/.hyperlane/chains/anvilchain2`.
...
:::tip
You should be able to see the messages of the contract deployments on your terminals running the local chains.
:::
...
🎉 There you go, you've sent a message between two local Anvil chains using Hyperlane!
