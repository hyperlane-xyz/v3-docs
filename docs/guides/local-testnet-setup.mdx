...
1. “Could not fetch metadata” warning:
   ...
   - **Solution:**
     Ensure the validators have announced their storage locations. Run the following command for each validator:
     ```bash
     hyperlane validator announce
     ```
     - Check validator logs for a message such as: `Validator has announced signature storage location, locations: ["file:///tmp/hyperlane-validator-signatures-local"].`
...
2. Messages time out:
   ...
   - **Solution:**
     - Make sure to use the `--block-time 1 flag` when starting Anvil to auto-mine blocks every second.
     - Alternatively, configure the validator and relayer to use 0 confirmations for faster processing during testing.
