...
Once finished, the CLI will create two new JSON artifact files: `warp-deployment` and `warp-config`. The first contains just addresses for the newly deployed Warp router contracts. The second is a config file for the Warp UI, which you can use for the next optional step.
...
Use the Hyperlane Warp UI app template for interacting with Warp Routes. Continue on to the [Warp UI docs](./deploy-warp-route-UI.mdx) for details on how to set it up.
