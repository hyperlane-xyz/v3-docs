...
Operators that want to quickly deploy a production ready environment using Docker Compose and AWS may wish to use the [Docker Compose quickstart guide](../operate/validators/docker-quickstart.mdx).
...
To confirm you are registed on mainnet, run `hyperlane avs check --chain ethereum` or `holesky` using the Hyperlane CLI and see if your validator address is there. This query may take a few minutes to complete.
