...
- Install the `@hyperlane-xyz/cli` package versioned `3.2.0-alpha` with `npm install -g @hyperlane-xyz/cli@3.2.0-alpha`. Follow more detailed instructions for installation [here](https://docs.hyperlane.xyz/docs/reference/cli).
...
Follow the validator guide [here](../operate/validators/run-validators.mdx) with the AWS setup to run the validator. Make sure your KMS key will have funds on Holesky to announce itself.
