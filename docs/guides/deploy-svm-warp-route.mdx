...
1. Deploy the warp route with `warp-route deploy`
    ...
    - The script is unlikely to work from the first try due to program size, but the script should be idempotent and skip contracts that were already deployed / initialized. Errors like `Error: 11 write transactions failed` can always be retried by re-running the command.
