...
2. Deploy the warp route using the `warp-route deploy` subcommand from the `rust/sealevel/client` directory:
   ...
   ```
    ...
    :::warning
    Please choose a reasonable amount to fund the Warp Route ATA payer accounts. The recommended initial funding amount is `alert_threshold * 2`, with the threshold value found [here](https://github.com/hyperlane-xyz/hyperlane-monorepo/blob/main/typescript/infra/src/warp/helm.ts).
    :::
...
:::warning
For production deployments, we strongly recommend avoiding the use of hotkeys like those used in this guide. Instead, transfer ownership to a **multisig setup**, such as [Squads](https://squads.so/), for enhanced security.
:::
