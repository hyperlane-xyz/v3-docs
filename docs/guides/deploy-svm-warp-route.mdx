...
3. Build the Warp Route programs on your machine
   ...
   To verify the build output, check the target/deploy directory for `.so` files: `ls ../../target/deploy`
...
2. Run the following command to deploy the warp route:
   ...
   Example usage with the required parameters:
...
1.  **Query the Warp Route Program**
    ...
    ```bash
    # run from `rust/sealevel/client`
    cargo run -- -k ./warp-route-deployer-key.json -u <CHAIN_RPC_URL> token query --program-id <PROGRAM_ID> synthetic
    ```
