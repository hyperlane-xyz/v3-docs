...
- The contract must ensure the tokens arrive before executing the follow-up action. Most calls requiring a token balance will revert if executed before the tokens arrive. The relayer retries with backoff to ensure successful ICA call execution.
