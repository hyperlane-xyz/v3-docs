# How to Use the Hyperlane Explorer
...
Messages to/from [core chains](/docs/reference/domains.mdx) will searchable by transaction hash, message ID, or sender/recipient address. For other chains, the explorer can be configured to search for messages by providing it a chain config.
...
![Explorer search bar](/img/explorersearchbar.png)
...
The explorer UI and REST API can be configured to search for messages on any chain with a Hyperlane deployment. See [configuring PI Chains](./configuring-pi-chains.mdx) for details.

### Debugging Messages

If you're trying to debug a problem with message delivery, the [explorer debugging](./explorer-debugging.mdx) page has useful tips.

### API Reference

The explorer's data can be accessed programmatically via the [REST API](./rest-api.mdx) or [GraphQL API](./graphql-api.mdx). The REST API is recommended because it exposes a simpler interface for message data.
